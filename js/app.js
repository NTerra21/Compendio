/**
 * Logica de la aplicación: galeria, ficha y navegacion por hash.
 * No edites personajes aqui: usa js/characters.js
 */
...
(function () {
  ...
  /** Retrato con clase; el fallback se enlaza despues del render. */
  ...
  /** Si la imagen falla, muestra un placeholder sin romper el layout. */
  function bindPortraitFallbacks(root) {
    root.querySelectorAll("img[data-fallback-name]").forEach(function (img) {
      ...
      // Si el recurso ya fallo antes de enlazar el listener
  ...
  function renderCharacter(id) {
    ...
    if (!character) {
      ...
      app.innerHTML =
        ...
        '<a class="btn" href="#/">Volver a la galeria</a>' +
        "</section>";
      return;
    }
